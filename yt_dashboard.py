...
# renaming the columns as they were having some ASCII values
    df_agg.columns = ['Video','Video title','Video publish time','Comments added','Shares','Dislikes','Likes',
                      'Subscribers lost','Subscribers gained','RPM(USD)','CPM(USD)','Average % viewed','Average view duration',
                      'Views','Watch time (hours)','Subscribers','Your estimated revenue (USD)','Impressions','Impressions ctr(%)']
    df_agg['Video publish time'] = df_agg['Video publish time'].str.replace('Jan', 'January')
    df_agg['Video publish time'] = df_agg['Video publish time'].str.replace('Feb', 'February')
    df_agg['Video publish time'] = df_agg['Video publish time'].str.replace('Mar', 'March')
    df_agg['Video publish time'] = df_agg['Video publish time'].str.replace('Apr', 'April')
    df_agg['Video publish time'] = df_agg['Video publish time'].str.replace('Jun', 'June')
    df_agg['Video publish time'] = df_agg['Video publish time'].str.replace('Jul', 'July')
    df_agg['Video publish time'] = df_agg['Video publish time'].str.replace('Aug', 'August')
    df_agg['Video publish time'] = df_agg['Video publish time'].str.replace('Sep', 'September')
    df_agg['Video publish time'] = df_agg['Video publish time'].str.replace('Oct', 'October')
    df_agg['Video publish time'] = df_agg['Video publish time'].str.replace('Nov', 'November')
    df_agg['Video publish time'] = df_agg['Video publish time'].str.replace('Dec', 'December')
# publish time was in string type in the dataset so converting it to a datetime value
    def parse_date(date_str): # function to change the datetime formmat
        try:
...
# calculating the avg view duration
...
# function to change the datetime formmat
    def parsing_date(date_str):
        try:
    ...
    df_time['Date'] = df_time['Date'].apply(parsing_date)
...
# ensuring to get the most recent data
metric_date_12mo = df_agg_diff['Video publish time'].max() - pd.DateOffset(months =12)

median_agg = df_agg_diff[df_agg_diff['Video publish time'].apply(lambda x: x >= metric_date_12mo)].median(numeric_only=True)

#create differences from the median for values 
# Create boolean mask for numeric columns
numeric_cols = np.array((df_agg_diff.dtypes == 'float64') | (df_agg_diff.dtypes == 'int64'))

# Subtract median_agg only from numeric columns and divide by median_agg
...
df_time_diff_yr = df_time_diff[df_time_diff['Video publish time'].apply(lambda x: x >= metric_date_12mo)]
...
if add_sidebar == 'Individual Video Analysis':
    st.write('Ken Jee Youtube Individual Video Data')
    video = tuple(df_agg['Video title'])
    video_select = st.selectbox('Pick a video:', video)
    
# now we will make the bar graph for the individuial video
    ...
    # passed the fig to the streamlit 
    st.plotly_chart(fig)
